...
See YAML above for the changes done
...
1. Order by year.
Not that interesting by year!  Try arranging by life expectancy.
...
Task 1: Make a data frame containing the columns `year`, `lifeExp`, `country` from the gapminder data, in that order.
Task 3: Order by year, then by life expectancy.

In the code below, gapminder is automatically used in the first argument for select()
