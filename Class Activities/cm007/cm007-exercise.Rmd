...
```
It only colours in the perimeters!  You need to not use the colour aesthetic, and use fill.
...
```{r}
#Free up the x-axis so they're not all the same; may help with interpretation
ggplot(gapminder, aes(lifeExp)) +
...
#Size is mapped to "character expansions"; to make it more meaningful, you can change it to be proportional to size of population
```{r}
...
gapminder %>%
  ...
  #scale_x_log10() + Not needed here as Rwanda's values are not great on log scale
  geom_point() +
...
cyl and am must be surrounded by factor functions; otherwise ggplot will treat them as numbers instead of categories
