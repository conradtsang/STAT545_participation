...
gapminder %>% 
  ...
  #Select the rows that are Canada AND 1952, then replace it with 70, otherwise all other rows give it regular lifeExp value
  filter(country == "Canada")
#This just so we filter to see Canada
```
...
# Summarize function groups numbers and returns one number
...
#Group by doesn't change the dataframe, but adds another layer
```{r}
gapminder %>% 
  group_by(country) %>% 
  summarize(mu = mean(lifeExp),
...
#There would be nothing left in the other columns; summarize function has compressed everything down, so it makes sense to get rid of them

For each continent: What is the smallest country-wide median GDP per capita?
#Remember, every time you do summarize, it "peels back" the layers by one
```{r}
gapminder %>% 
  group_by(continent, country) %>% 
...
#If you want it to have a better labelling:
```{r}
gapminder %>% 
...
#Literally "lags" the data by 1, e.g. (5,2,1) turns into (NA,5,2)
```
...
gapminder %>% 
  ...
  filter(!is.na(gain)) %>%
  #! is the symbol for means not
  summarize(min = min(gain)) %>% 
  summarize(min = min(min)) %>% 
  arrange(min)
#The smallest gain is the "sharpest drop"
```

2. Compute the relative gdp (NOT per capita!) of each country compared to Canada (= GDP of a country / GDP of Canada).
#There are multiple ways to do this, including using Table Joins which we will learn next week!
#The below is not the most sane way to do it, but it works to get an answer
```{r}
cda <- gapminder %>% 
  mutate(gdp = gdpPercap * pop) %>% 
  filter(country == "Canada") %>% 
