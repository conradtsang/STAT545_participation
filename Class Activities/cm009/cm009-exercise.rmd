...
#~ indicates to tribble function that these are the words we are using
```{r}
tribble(
  ~name, ~time,
  "Frank", 30,
...
```
#write_csv(iris, "") you can press Tab to see all files in the current working directory
...
```
#Not tidy as gender is split into two variables
...
#To bring an unknown amount of data in, make a list
#do.call(f, l) will apply the function f to all components of l (and you don't know how many l there are)
...
```
#Here we see NA's because the Words variable has some non-unique combinations.  It is possible to remove them too
