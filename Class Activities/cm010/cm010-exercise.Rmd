...
### what happen if colnames are different in `y` and `x`? Is there any error message and why?
#Error message because in the set function both tables have to have identical column names.  Can use mutating functions
```{r}
...
#dplyr changes the column name, as it cannot have the same column name in two columns
```{r}
bind_cols(y,z)
```
